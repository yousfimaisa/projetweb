...
// Données générales
$totalPays = $payController->countPays();
$totalFactures = count($factureController->getAllFactures());

// Répartition des types de cartes
$types = ['Visa', 'MasterCard', 'American Express', 'Discover'];
$typeCounts = [];
foreach ($types as $type) {
    $typeCounts[] = count($payController->listPays($type));
}

// Montant total et moyen des factures
$factures = $factureController->getAllFactures();
$montantTotal = array_sum(array_column($factures, 'amount'));
$montantMoyen = $totalFactures > 0 ? $montantTotal / $totalFactures : 0;

// Derniers paiements et factures
$recentPays = array_slice($payController->listPay(), -5);
$recentFactures = array_slice($factures, -5);
?>
  ...
  .dark-mode .navbar, .dark-mode .card, .dark-mode .sidenav {
    background-color: #1f1f1f !important;
    color: #e0e0e0 !important;
  }
  .dark-mode .nav-link, .dark-mode .nav-link i {
    color: #e0e0e0 !important;
  }
    </style>
...
<div class="container my-5">
    <h2 class="mb-4 text-center">📊 Tableau de Bord - Statistiques</h2>
    ...
    <div class="row text-center mb-4">
        ...
        <div class="col-md-3">
            <div class="card shadow-sm">
                <div class="card-body">
                    <h5>Montant Total (€)</h5>
                    <h3><?= number_format($montantTotal, 2) ?></h3>
                </div>
            </div>
        </div>
        <div class="col-md-3">
            <div class="card shadow-sm">
                <div class="card-body">
                    <h5>Montant Moyen (€)</h5>
                    <h3><?= number_format($montantMoyen, 2) ?></h3>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="row">
        <div class="col-md-6">
            <canvas id="typeCarteChart"></canvas>
        </div>
        <div class="col-md-6">
        ...
        <div class="col-md-6">
            ...
            <ul class="list-group">
                <?php foreach ($recentFactures as $facture): ?>
                    <li class="list-group-item">
                        <?= $facture['description'] ?> - <?= $facture['amount'] ?> €
                    </li>
                <?php endforeach; ?>
            </ul>
        </div>
...
<script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
<script>
const ctx1 = document.getElementById('typeCarteChart').getContext('2d');
new Chart(ctx1, {
    type: 'doughnut',
    data: {
    ...
    options: {
        plugins: {
            title: { display: true, text: 'Répartition des Types de Cartes' }
        }
    }
});

const ctx2 = document.getElementById('factureChart').getContext('2d');
new Chart(ctx2, {
    type: 'bar',
    data: {
        labels: ['Visa', 'MasterCard', 'American Express', 'Discover'],
    ...
    options: {
        plugins: {
            title: { display: true, text: 'Factures par Type de Carte' }
        }
    }
});
</script>
