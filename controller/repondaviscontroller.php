...
class RepondAvisController
{
    ...
    public function getReviewStatistics() {
        // 1. Nombre total d'avis
        $queryTotal = "SELECT COUNT(*) as total FROM avis";
        $stmtTotal = $this->db->prepare($queryTotal); // Utilisez votre connexion existante
        $stmtTotal->execute();
        $totalReviews = $stmtTotal->fetch(PDO::FETCH_ASSOC)['total'];
        
        // 2. Note moyenne
        $queryAvg = "SELECT AVG(note) as moyenne FROM avis";
        $stmtAvg = $this->db->prepare($queryAvg);
        $stmtAvg->execute();
        $averageRating = round($stmtAvg->fetch(PDO::FETCH_ASSOC)['moyenne'], 1);
        
        // 3. Données hebdomadaires pour le graphique (7 derniers jours)
        $queryWeekly = "SELECT 
                        DATE(date_avis) as jour, 
                        COUNT(*) as nombre, 
                        AVG(note) as moyenne_note
                        FROM avis
                        WHERE date_avis >= DATE_SUB(CURDATE(), INTERVAL 7 DAY)
                        GROUP BY DATE(date_avis)
                        ORDER BY jour ASC";
        
        $stmtWeekly = $this->db->prepare($queryWeekly);
        $stmtWeekly->execute();
        ...
        foreach ($weeklyResults as $row) {
            $labels[] = date('d/m', strtotime($row['jour']));
            $values[] = (int)$row['nombre'];
            $averageNotes[] = (float)$row['moyenne_note'];
        }
